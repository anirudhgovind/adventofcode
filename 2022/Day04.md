...
input <- read_lines(here::here("data/rawData/04.txt"),
                    skip_empty_rows = T)
```
...
# Separate into a column for each elf
...
# Further separate into start and end values for each elf

df <- df %>% 
  separate(elf1,
           into = c("elf1Start", "elf1End"),
           sep = "-") %>% 
  separate(elf2,
           into = c("elf2Start", "elf2End"),
           sep = "-") %>% 
  mutate(elf1Start = as.integer(elf1Start),
         elf1End = as.integer(elf1End),
         elf2Start = as.integer(elf2Start),
         elf2End = as.integer(elf2End))

# Filter cases where one set of assignments is contained in the other

df %>% 
  filter((elf1Start >= elf2Start & elf1End <= elf2End) | 
...
``` r
df %>% 
  filter((elf1Start %in% range(df$elf2Start, df$elf2End)) |
         (elf1End %in% range(df$elf2Start, df$elf2End)) | 
         (elf2Start %in% range(df$elf1Start, df$elf1End)) |
         (elf2End %in% range(df$elf1Start, df$elf1End))) %>% 
  view()

df <- df %>% 
  mutate(overlap = case_when(elf2Start <= elf1Start & elf1Start <= elf2End ~ "Y",
                             elf1Start <= elf2Start & elf2Start <= elf1End ~ "Y",
                             elf1Start <= elf2End & elf2End <= elf1End ~ "Y",
                             elf2Start <= elf1End & elf1End <= elf2End ~ "Y",
                             TRUE ~ "N"))
